...
int query ( int nowL, int nowR, int l, int r, int n ) {
	if ( l <= nowL && nowR <= r )
		return seg[n];
	int mid = ( nowL + nowR ) >> 1, leftSon = n << 1, rightSon = leftSon | 1;
	if ( r <= mid )
		return query ( nowL, mid, l, r, leftSon );
	if ( mid < l )
		return query ( mid + 1, nowR, l, r, rightSon );

	return max ( query ( nowL, mid, l, r, leftSon ), query ( mid + 1, nowR, l, r, rightSon ) );
}
